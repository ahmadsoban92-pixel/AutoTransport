              ...
              {ALL_STATUSES.map((status) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border text-sm font-medium transition-all ${
                    newStatus === status ? STATUS_COLORS[status] : "border-blue-800/30 text-blue-400 hover:border-blue-600/50 hover:text-blue-200"
                  }`}
                >
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${
                    status === "New" ? "bg-orange-400" : status === "Contacted" ? "bg-yellow-400" :
                    status === "Quoted" ? "bg-purple-400" : status === "Booked" ? "bg-green-400" : "bg-red-400"
                  }`} />
                  {status}
                </button>
              ))}

...
import { Truck, Target, Users, Award, Shield } from "lucide-react";
...
const values = [
  { icon: <Shield className="w-6 h-6 text-orange-400" />, title: "Trust & Transparency", desc: "No hidden fees. No bait-and-switch pricing. We tell you exactly what you're paying for, upfront." },
  { icon: <Target className="w-6 h-6 text-orange-400" />, title: "Reliability", desc: "98.7% on-time delivery rate. We set realistic timelines and we meet them — every single time." },
  { icon: <Users className="w-6 h-6 text-orange-400" />, title: "Customer First", desc: "From first contact to final delivery, our brokers are only a call away to answer your questions." },
  { icon: <Award className="w-6 h-6 text-orange-400" />, title: "Excellence", desc: "Thousands of repeat customers and a proven track record of successful shipments speak for our commitment to quality." },
];
    ...
    <div className="min-h-screen bg-[#060d1f] pt-28 pb-16">
      ...
      <section className="py-20 px-6 text-center">
        <div className="max-w-4xl mx-auto">
          ...
          <motion.h1 {...fadeUp(0.1)} className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-6 break-words">
            About <span className="text-orange-400">WESAutoTransport</span>
          </motion.h1>
          <motion.p {...fadeUp(0.2)} className="text-lg sm:text-xl text-blue-300 leading-relaxed max-w-3xl mx-auto">
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="relative pl-7 sm:pl-8 border-l border-blue-800/50 space-y-10">
            {milestones.map((m, i) => (
              <motion.div key={m.year} {...fadeUp(i * 0.1)} className="relative">
                <div className="absolute -left-[1.85rem] sm:-left-[2.15rem] w-4 h-4 rounded-full bg-orange-500 border-2 border-[#060d1f]" />
                <div className="text-orange-400 text-sm font-bold mb-1">{m.year}</div>
                <p className="text-blue-200 text-sm sm:text-base">{m.event}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {values.map((v, i) => (
              <motion.div key={v.title} {...fadeUp(i * 0.1)}
                ...
                <div className="w-12 h-12 rounded-xl bg-orange-500/10 flex items-center justify-center mb-4">{v.icon}</div>
      ...
      <section className="py-20 px-6 text-center">
        <motion.div {...fadeUp(0)} className="max-w-2xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/get-quote">
              <Button className="bg-orange-500 hover:bg-orange-600 text-white border-0 h-12 px-8 text-base w-full sm:w-auto">
                Get Your Free Quote →
              </Button>
            </Link>
            <a href="https://wa.me/923059846727" target="_blank" rel="noopener noreferrer">
              <Button className="h-12 px-8 text-base bg-green-600 hover:bg-green-700 text-white border-0 w-full sm:w-auto">
                WhatsApp Us
              </Button>
            </a>
          </div>
